...
const AuthPage: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let response;
      if (isLogin) {
        response = await login({ email: formData.email, password: formData.password }).unwrap();
      } else {
        const { confirmPassword, ...registerData } = formData;
        response = await register(registerData).unwrap();
      }
        ...
        <form onSubmit={handleSubmit}>
          ...
          <button
            ...
            style={{
              ...
              fontWeight: 600,
              cursor: 'pointer',
              opacity: (loginLoading || registerLoading) ? 0.7 : 1
            }}
          >
            {loginLoading || registerLoading ? 'Загрузка...' : (isLogin ? 'Войти' : 'Зарегистрироваться')}
          </button>
        </form>
